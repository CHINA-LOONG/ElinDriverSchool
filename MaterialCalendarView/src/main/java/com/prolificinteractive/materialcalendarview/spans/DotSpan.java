...
public class DotSpan implements LineBackgroundSpan {
    ...
    public DotSpan(float radius, int color, Bitmap bitmap) {
        this.radius = radius;
        this.color = color;
        this.bitmap = bitmap;
    }
...
//        canvas.drawCircle((left+right)/2+4*radius, top + radius, radius, paint);
//                    canvas.drawCircle((left + right) / 2, bottom + radius, radius, paint);

        paint.setColor(oldColor);

        canvas.drawBitmap(bitmap,(left+right)/2+6*radius, top-6*radius,paint);
